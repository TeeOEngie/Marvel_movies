...
# ============================================================
with tab_howto:
    st.subheader("แอปนี้ทำอะไร")
    st.markdown(
        "แอปนี้ทำนาย **รายได้รวมทั่วโลก (worldwide box office)** ของหนัง MCU "
        "โดยใช้โมเดล **Random Forest Regressor** ที่เรียนรู้แพทเทิร์นจากหนัง MCU "
        "34 เรื่องที่ผ่านมา — กรอกข้อมูลหนัง (สมมติหรือหนังจริงที่ยังไม่ฉาย) "
        "แล้วโมเดลจะประมาณรายได้ที่น่าจะทำได้ให้"
    )

    st.write("")
    st.subheader("ขั้นตอนใช้งาน")
    st.markdown(
        """
        1. ไปที่แท็บ **🔮 ทำนาย**
        2. กรอกข้อมูลหนังทางฝั่งซ้ายให้ครบทั้ง 8 ช่อง
        3. กดปุ่ม **🎬 ทำนายรายได้**
        4. ดูผลลัพธ์ทางฝั่งขวา — รายได้ที่คาดการณ์, ช่วงความเป็นไปได้ 80%,
           กำไร/ROI โดยประมาณ และกราฟแสดงผลโหวตของแต่ละต้นไม้ในโมเดล
        """
    )

    st.write("")
    st.subheader("แต่ละช่องกรอกใช้ทำอะไร")

    input_help = [
        ("💰 งบสร้าง (Production Budget)",
         "งบประมาณสร้างหนัง หน่วยล้านดอลลาร์ — ยิ่งงบสูง มักสัมพันธ์กับรายได้ที่สูงตาม "
         "(หนังทุนสูงมักมีเอฟเฟกต์/นักแสดงดัง/การตลาดที่ใหญ่กว่า)"),
        ("⏱️ ความยาวหนัง (Runtime)",
         "ความยาวของหนัง หน่วยนาที — ส่งผลทางอ้อมต่อรายได้ เพราะเกี่ยวกับจำนวนรอบฉายต่อวันที่โรงหนังจัดได้"),
        ("🎬 เฟส MCU (Phase)",
         "หนังอยู่ในเฟสไหนของจักรวาล MCU (1-6) — สะท้อนช่วงเวลาและกระแสความนิยมของแฟรนไชส์ในตอนนั้น"),
        ("🔢 ลำดับการฉาย (Timeline Order)",
         "ลำดับที่หนังเรื่องนี้ฉายเทียบกับหนัง MCU เรื่องอื่นๆ ทั้งหมด — ช่วยบอกตำแหน่งในไทม์ไลน์การฉายทั้งหมด"),
        ("📅 ปีที่ฉาย (Release Year)",
         "ปี ค.ศ. ที่หนังเข้าฉาย — ราคาตั๋วหนังและพฤติกรรมผู้ชมเปลี่ยนไปตามยุคสมัย"),
        ("⭐ คะแนน IMDb",
         "คะแนนรีวิวจากผู้ชมทั่วไปใน IMDb (เต็ม 10) — เป็นตัวแปรที่ส่งผลต่อการทายมากที่สุดในโมเดลนี้ "
         "เพราะสะท้อนว่าหนังถูกใจคนดูแค่ไหน"),
        ("🍅 คะแนน Rotten Tomatoes",
         "คะแนนจากนักวิจารณ์ภาพยนตร์ (เต็ม 100%) — สะท้อนคุณภาพหนังในมุมมองมืออาชีพ"),
        ("🏆 คะแนน Metacritic",
         "คะแนนเฉลี่ยจากนักวิจารณ์หลายสำนัก (เต็ม 100) — คล้าย Rotten Tomatoes แต่คำนวณคนละวิธี"),
    ]

    for name, desc in input_help:
        with st.expander(name):
            st.write(desc)

    st.write("")
    st.subheader("อ่านผลลัพธ์ยังไง")
    st.markdown(
        """
        - **รายได้ที่คาดการณ์** — ตัวเลขหลักบนการ์ดสีแดง คือค่าเฉลี่ยจากทุกต้นไม้ในโมเดล
        - **ช่วงความเป็นไปได้ 80%** — บอกว่าต้นไม้ส่วนใหญ่ทายอยู่ในช่วงไหน ยิ่งช่วงกว้าง
          แปลว่าโมเดลไม่ค่อยมั่นใจ (ต้นไม้แต่ละต้นเห็นต่างกันเยอะ)
        - **กำไร/ROI โดยประมาณ** — คำนวณจาก (รายได้ที่ทาย - งบสร้างที่กรอก)
        - **กราฟฮิสโตแกรม** — โชว์ว่าต้นไม้แต่ละต้น (จากทั้งหมดในป่า) ทายค่าออกมาเท่าไหร่บ้าง
          ก่อนจะเฉลี่ยรวมเป็นคำตอบสุดท้าย
        """
    )

    st.markdown(
        '<div class="caveat-box">⚠️ ข้อควรระวัง: โมเดลเทรนจากข้อมูลแค่ 34 เรื่อง '
        "ถ้ากรอกค่าที่ห่างไกลจากข้อมูลจริงมาก (เช่น งบสร้างสูงผิดปกติ) "
        "ผลทายจะยิ่งไม่น่าเชื่อถือ ควรมองเป็นการประมาณคร่าวๆ เพื่อความสนุก ไม่ใช่ตัวเลขจริงจัง</div>",
        unsafe_allow_html=True,
    )
...
with tab_dev:
    ...
    st.markdown(
        "โครงการนี้จัดทำขึ้นเพื่อพัฒนาระบบทำนายรายได้รวมทั่วโลก (Worldwide Box Office) ของภาพยนตร์ใน"
        "จักรวาลภาพยนตร์มาร์เวล (Marvel Cinematic Universe: MCU) โดยประยุกต์ใช้เทคนิคการเรียนรู้ของ "
        "เครื่องแบบกลุ่ม (Ensemble Learning) ด้วยอัลกอริทึม Random Forest Regressor ซึ่งอาศัยการรวมผล"
        "การทำนายจากต้นไม้ตัดสินใจ (Decision Tree) จำนวนมากเพื่อเพิ่มความแม่นยำและลดปัญหาการเรียนรู้เกิน "
        "พอดี (Overfitting) เมื่อเทียบกับการใช้ต้นไม้ตัดสินใจเพียงต้นเดียว"
    )
